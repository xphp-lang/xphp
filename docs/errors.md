...
| `xphp.alias_unsupported_body` | a type-alias body that is not a single head, a union, an intersection, a nullable, a DNF, or a closure signature — e.g. a closure signature combined with a union (`A \| Closure(int): int`) or nullable (`?Closure(int): int`) |
