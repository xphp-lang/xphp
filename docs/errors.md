...
| `xphp.unspecialized_generic_closure` | a generic closure/arrow is declared but never turbofish-called, so nothing grounds its type parameters — the emitted value would keep raw hints naming non-existent classes (`App\T`) and fatal on first invocation, even when only handed away as a callable. Call it with `$var::<...>(...)`, or remove the `<...>` clause (also flagged when the parameters are never referenced — the clause is dead syntax) |
