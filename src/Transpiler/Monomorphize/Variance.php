 ...
 *
 *  - `Invariant` (the default, no marker): T can appear in both input and
 *    output positions; specializations are unrelated unless their args are
 *    identical.
 *  - `Covariant` (`out T`): T can appear in method return positions (and in a
 *    plain constructor parameter — see below). `T1 <: T2` lifts to
 *    `Box<T1> <: Box<T2>`.
 *  - `Contravariant` (`in T`): T can appear in method parameter positions (and in
 ...
 * strict-invariant for both `out T` and `in T`: PHP enforces invariant property types
